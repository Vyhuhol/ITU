...
namespace DocflowApp.App_Start
{
    public partial class Startup
    {
        public static void Configuration(IAppBuilder app)
        {
            ...
            var userRepository = new UserRepository(DependencyResolver.Current.GetServices<ISession>().FirstOrDefault());
            var f = new UserFilter() { UserName = "admin" };

            if (userRepository.Count(f) == 0)
            {
                var userManager = new UserManager(
                new IdentityStore(DependencyResolver.Current.GetServices<ISession>().FirstOrDefault()));
                userManager.CreateAsync(new User { UserName = "admin" }, "12345");
            }
        }
    }
}